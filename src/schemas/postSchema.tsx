...
export const validateSummary = z.string().superRefine((value, ctx) => {
    ...
    if (value.length > 1000) {
        ctx.addIssue({
            code: 'custom',
            message: 'Ringkasan maksimal 1000 karakter',
        });
    }
});
...
export const validateCategoryID = z.number().superRefine((value, ctx) => {
    ...
    if (!Number.isInteger(value) || value <= 0) {
        ctx.addIssue({
            code: 'custom',
            message: 'Kategori harus berupa angka positif',
        });
    }
});

export const validateOptionalUserID = z
    .number()
    .optional()
    .superRefine((value, ctx) => {
        if (value !== undefined && (!Number.isInteger(value) || value < 0)) {
            ctx.addIssue({
                code: 'custom',
                message: 'User harus berupa angka positif',
            });
        }
    });
