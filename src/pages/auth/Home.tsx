...
import useSearchPost from '../../hooks/useSearchPost.ts';
...
import { getDateRangeInUnix } from '../../utils/postUtils.ts';
...
const Home = () => {
    ...
    const barData = {
        labels: data.map((item: PostData) =>
            item.title.length > 15 ? truncateText(item.title, 15) : item.title
        ),
        datasets: [
            {
