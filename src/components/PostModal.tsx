...
const PostModal = ({
    ...
    const handleEditorChange = useCallback((e: CustomEditorChangeEvent) => {
        const htmlValue = e.htmlValue || "";
        editorContentRef.current = htmlValue;
    }, [editorContentRef]);

    const handleEditorLoad = useCallback((quillInstance: Quill) => {
        if (quillInstance) {
            const toolbar = quillInstance.getModule('toolbar') as { container: HTMLElement } | null;
    ...
    const { isMenuVisible, setIsMenuVisible, toggleMenuVisibility, key: menuKey, buttonRef, menuContainerRef } = useSidebar();
    ...
    useEffect(() => {
        ...
        if (data?.thumbnail || croppedImage) {
            items.push({
                ...
                command() {
                    setIsMenuVisible(false);
                    if (setData) {
                        setData(prev => ({ ...prev, thumbnail: "", deleteThumbnail: true }));
                    }
        ...
        setMenuItems(items);
    }, [data?.thumbnail, croppedImage, handleClickUploadButton, setData, setThumbnail, setCroppedImage, setIsMenuVisible]);
    ...
    }, [editorKey, isUploadingImage, quillModules, headerTemplate, handleEditorLoad, handleEditorChange, editorContentRef]);

    return (
        <Dialog
            closable={!submitLoading && !isUploadingImage}
            header={<h1 className="font-medium m-0 text-xl">{isEditMode ? "Edit Berita" : "Buat Berita Baru"}</h1>}
            visible={visible}
            maximizable
        ...
        >
            <form onSubmit={handleSubmit} className="w-full">
                <div className="flex flex-col p-4 gap-4">
                    ...
                    {role == "admin" ? (
                        <div className="w-full">
                            <label htmlFor="userID" className="block mb-1 font-medium">Penulis</label>
                            <Dropdown
                                id="userID"
                                className="w-full"
                                filter
                    ...
                    <div className="w-full flex flex-col ">
                        <label htmlFor="name" className="block mb-1 font-medium">Thumbnail</label>
                    ...
                    <div className="w-full">
                        <label htmlFor="summary" className={`block mb-1 font-medium ${errors.summary ? "p-error" : "text-[#48525f]"}`}>Ringkasan</label>
                        <InputTextarea
                            autoResize={true}
                            id="summary"
                    ...
                    <div className="w-full ">
                        <label htmlFor="content" className="block mb-1 font-medium">Konten</label>
                        <div className={`w-full h-fit relative`}>
                            {isUploadingImage && (
                                <div className="loading-container">
                                    <i className="pi pi-spin pi-spinner text-[3rem]" style={{ color: "#64748b", animationDuration: "1s" }}></i>
                                </div>
                            )}
